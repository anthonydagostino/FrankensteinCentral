...
(function () {
  ...
  function renderMoney(m, bud) {
    ...
    q("#cc-money").innerHTML = `
      ...
      ${staleLine}
      <div class="mny-hero">
        <div class="mny-stat"><div class="v">${m.month != null ? `${m.month_is_lower_bound ? "≥ " : ""}${money(m.month)}` : "—"}</div><div class="l">${m.month_label ? `Spent in ${esch(m.month_label.split(" ")[0])}` : "Spent this month"}<br><span style="font-size:10px">${m.month_is_lower_bound ? "at least — part of the ledger couldn't be read" : "month to date"}${m.month_savings ? ` · ${money(m.month_savings)} to savings not counted` : ""}</span></div></div>
