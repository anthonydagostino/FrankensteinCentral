...
(function () {
  ...
  // ---- Weekly review --------------------------------------------------------
  // GET /weekly-review was built and had zero frontend references.
  function renderWeeklyReview(w) {
    ...
    const st = w.study || {}, gy = w.gym || {}, wa = w.water || {};
    const bar = (label, done, goal, unit) => {
      const known = done != null && goal;
      const pct = known ? Math.min(100, Math.round((done / goal) * 100)) : 0;
      return `<div class="wk-row${known ? "" : " untracked"}">
          <span>${esch(label)}</span>
          <div class="track"><div class="fill" style="width:${pct}%"></div></div>
          <em>${known ? `${done}${unit} / ${goal}${unit}` : "not tracked"}</em>
        </div>`;
    };
    const delta = (st.week_min != null && st.last_min != null)
      ? st.week_min - st.last_min : null;
    const deltaTxt = delta == null ? ""
      : ` <span class="wk-delta ${delta >= 0 ? "up" : "down"}">${delta >= 0 ? "+" : ""}${Math.round(delta / 60 * 10) / 10}h vs last week</span>`;
    el.hidden = false;
    el.innerHTML = `<h3>This week${deltaTxt}</h3>`
      + bar("Study", st.week_min, st.goal_min, "m")
      + bar("Gym", gy.week, gy.goal, "")
      + bar("Water", wa.days_hit, wa.of, " days");
  }
