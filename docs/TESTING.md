...
| `services/assistant/tests/test_runway.py` | 29 | cash runway: a brokerage is not grocery money, a liability never joins the pot, an unroled account is neither counted nor dropped (the figure becomes a lower bound), and a truncated or stale read yields null rather than a longer runway — every suppression fails optimistically if removed; plus SCRUM-93, replayed against the eleven real accounts that produced a live 4.9x overstatement — a uniform role field is a default, not a classification, and a negative "asset" is a debt |
| `services/assistant/tests/test_answers.py` | 27 | `/ask` never states a figure it didn't read: every intent, with its source down, names the service instead of rendering the outage as $0; "empty" and "unreachable" stay distinguishable; and specific intents aren't shadowed by broad ones |
| `tests/test_vault_serve_examples.py` | 321 | the `bw serve` examples in every doc, `.env.example` and the service docstring point at the docker bridge, never 0.0.0.0 or the CLI's colliding default port — and (SCRUM-133) the guard never reads a machine's live `.env`, which made it green in CI and red on the box, blocking every deploy |
| `tests/test_wire_names.py` | 53 | one concept, one key name across every service boundary: the completeness flag is `window_complete` everywhere, no service reads or publishes another spelling, it never moves back inside firefly's `window` object, and every producer still publishes it — a name guard alone would pass if the flag disappeared |
